...
uptimemon.py -p program1=200MB -p theprog:thegroup=100MB -g thegroup=100MB -a 1GB -s "/usr/sbin/sendmail -t -i" -m chrism@plope.com
"""
...
class Memmon:
    ...
    def restart(self, name, rss):
        ...
        try:
            ...
            if self.email:
                subject = 'uptimemon: failed to stop process %s, exiting' % name
        ...
        try:
            ...
            self.stderr.write(str(msg))
            if self.email:
                subject = 'uptimemon: failed to start process %s, exiting' % name
                self.mail(self.email, subject, msg)
            raise
        ...
        if self.email:
            now = time.asctime()
            msg = (
                'uptimemon.py restarted the process named %s at %s because '
                'it was consuming too much uptimeory (%s bytes RSS)' % (
                name, now, rss)
                )
            subject = 'uptimemon: process %s restarted' % name
            self.mail(self.email, subject, msg)
...
def main():
    ...
    uptimemon = Memmon(programs, groups, any, sendmail, email, rpc)
